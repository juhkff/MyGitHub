...
import module.Project_In_Accepted;
import module.User;
...
public class Project_Accepted {
    ...
    public ArrayList<Project_In_Accepted> getAcceptedProjectList(String userId){
        ...
        String sql2;
        if (teamName!=null){
            sql2="SELECT ProName,ProFounder,SettedTime,ProType,IsSended,IsPassed FROM projects WHERE RequiredName=? OR RequiredName=?";
            try {
            ...
            sql2="SELECT ProName,ProFounder,SettedTime,ProType,IsSended,IsPassed FROM projects WHERE RequiredName=?";
            try {
                preparedStatement=connection.prepareStatement(sql2);
                preparedStatement.setString(1,userName);
            } catch (SQLException e) {
        ...
        try {
            ResultSet resultSet=preparedStatement.executeQuery();
            while (resultSet.next()) {
                if (resultSet.getString(6)!=null&&resultSet.getString(6).equals("1")) {     //error1:   !=null&&
                    continue;
                } else {
    ...
    public String quitProject(String proName){
        ...
        Connection connection=myConnection.getConnection();
        PreparedStatement preparedStatement=null;
        String sql="UPDATE projects SET ProType=null,IsRequired=?,RequiredName=null WHERE ProName=?";
        int result=-1;
        try {
            preparedStatement=connection.prepareStatement(sql);
